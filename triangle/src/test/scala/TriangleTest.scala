...
class TriangleTest extends FunSuite with Matchers {
  ...
  test(
    "equilateral - All zero sides are illegal, so the triangle is not equilateral"
  ) {
  ...
  test(
    "isosceles - Sides that violate triangle inequality are not isosceles, even if two are equal"
  ) {
  ...
  test(
    "scalene - Sides that violate triangle inequality are not scalene, even if they are all different"
  ) {
